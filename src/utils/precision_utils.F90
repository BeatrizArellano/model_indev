...
contains
   ...
   pure function get_nan_rk() result(x)
      real(rk) :: x
      x = ieee_value(0.0_rk, ieee_quiet_nan)
   end function get_nan_rk

   pure logical function is_nan_rk(x)
      real(rk), intent(in) :: x
      is_nan_rk = ieee_is_nan(x)
   end function is_nan_rk

   pure elemental logical function is_finite_rk(x)
      real(rk), intent(in) :: x
      is_finite_rk = ieee_is_finite(x)
   end function
