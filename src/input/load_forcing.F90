...
contains
      ...
      do i = 1, size(forcmd%vars)
         if (forcmd%vars(i)%input_type == in_constant) then
            ...
            case ('surf_air_temp')
               ...
               Y%air_temp%idx = 1; Y%air_temp%n = 0; Y%air_temp%t_next = huge(1.0_rk)
            ...
            case ('sl_pressure')
               ...
               Y%slp%idx = 1; Y%slp%n = 0; Y%slp%t_next = huge(1.0_rk)
            ...
            case ('relative_humidity')
               ...
               Y%rel_hum%idx = 1; Y%rel_hum%n = 0; Y%rel_hum%t_next = huge(1.0_rk)
            ...
            case ('shortwave_radiation')
               ...
               Y%short_rad%idx = 1; Y%short_rad%n = 0; Y%short_rad%t_next = huge(1.0_rk)
            ...
            case ('longwave_radiation')
               ...
               Y%long_rad%idx = 1; Y%long_rad%n = 0; Y%long_rad%t_next = huge(1.0_rk)
            ...
            case ('wind_speed')
               ...
               Y%wind_spd%idx = 1; Y%wind_spd%n = 0; Y%wind_spd%t_next = huge(1.0_rk)
            ...
            case ('wind_direction')
               ...
               Y%wind_dir%idx = 1; Y%wind_dir%n = 0; Y%wind_dir%t_next = huge(1.0_rk)
            ...
            case ('co2_air')
               ...
               Y%co2_air%idx = 1; Y%co2_air%n = 0; Y%co2_air%t_next = huge(1.0_rk)
            case default
   ...
   real(rk) function value_at_step(self, t) result(v)
      ...
      nloc = self%n
      ! advance cursor only when we cross a forcing timestamp
      do while (self%idx < nloc .and. t >= self%t_next)
         self%idx = self%idx + 1
