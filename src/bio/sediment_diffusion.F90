...
!   boundary conditions in the sediment
!
!=======================================================================================
module sediment_diffusion
  use precision_types, only: rk
  use tridiagonal, only: TridiagCoeff, solve_tridiag, reset_tridiag
  implicit none
  private

  public :: scalar_diffusion_sed
...
contains
    ...
    !   - phase_thickness(1:N):
    !          Storage capacity of the transported phase in each layer 
    !          per unit horizontal area [m].
    !          This is because Diffusion occurs through pore (or solid) space so effective cross-sectional area is reduced by φ (or 1-φ)
    !          = φ(i)*h(i) for porewater
    !          = (1-φ(i))*h(i) for solids
    !   - diff(0:N)  are interface diffusivities [m2 s-1], bottom..top.
    !       * This should be already scaled by porosity
    !         = φ(i)*D_eff(i) for porewater
    !         = (1-φ(i))*Db(i) for solids
    !       * Only diff(1..N-1) (internal interfaces) are actually used here.
    !   - Boundary conditions:
    ...
    subroutine scalar_diffusion_sed(Var, N, dt, h, phase_thickness, diff, cnpar, tricoef,  &
                                    ierr, enforce_nonneg,                 &
        ...
        real(rk),           intent(in)    :: phase_thickness(1:N)      ! Phase-specific storahe capacity per unit horizontal area
        ...
        !---------------------------
        do i = 2, N-1
            c = 2.0_rk*dt*diff(i)/(h(i)+h(i+1))/phase_thickness(i)     ! couples to i+1
            a = 2.0_rk*dt*diff(i-1)/(h(i)+h(i-1))/phase_thickness(i)   ! couples to i-1
        ...
        select case (bot_type)
            case (BC_NEUMANN)
                !   bot_val is the flux INTO the bottom layer: mass flux per total horizontal area [M m^-2 s^-1]
                !   (positive = into the layer, negative = out of the domain)
                c = 2.0_rk* dt * diff(1)/(h(1)+h(2))/phase_thickness(1)
                ...
                if (do_nonneg .and. bot_val < 0.0_rk) then
                    ...
                    tricoef%bu(1) = 1.0_rk - tricoef%cu(1) - dt*bot_val / (max(Var(1),tinyV)*phase_thickness(1))
                    tricoef%du(1) = Var(1) + (1.0_rk-cnpar)*c*(Var(2)-Var(1))
                else
                    tricoef%bu(1) = 1.0_rk - tricoef%cu(1)
                    tricoef%du(1) = Var(1) + (1.0_rk-cnpar)*c*(Var(2)-Var(1)) + dt*bot_val/phase_thickness(1)
                end if
        ...
        select case (top_type)
            case (BC_NEUMANN)
                ...
                !   (positive = into the layer, negative = out of the domain)
                a = 2.0_rk * dt * diff(N-1)/(h(N)+h(N-1))/phase_thickness(N)
                ...
                if (do_nonneg .and. top_val < 0.0_rk) then
                    tricoef%bu(N) = 1.0_rk - tricoef%au(N) - dt*top_val / (max(Var(N),tinyV)*phase_thickness(N))
                    tricoef%du(N) = Var(N) + (1.0_rk-cnpar)*a*(Var(N-1)-Var(N))
                else
                    tricoef%bu(N) = 1.0_rk - tricoef%au(N)
                    tricoef%du(N) = Var(N) + (1.0_rk-cnpar)*a*(Var(N-1)-Var(N)) + dt*top_val/phase_thickness(N)
                end if
    ...
    end subroutine scalar_diffusion_sed


end module sediment_diffusion
