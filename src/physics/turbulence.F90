...
contains
    ...
    subroutine TURBULENCE_ke(N, dt, params, h, density, velx, vely, &
        ...
        do i=0,N
            LLk  = (Lscale(i)*Lscale(i)) / max(tke(i),tke_min)
            as(i)= LLk * SS(i)
            an(i)= LLk * NN(i)
        end do
        ...
        do i=0,N
            ...
            Nz(i) = Nz(i) + mol_nu
            Kz(i) = Kz(i) + mol_nu
            if (Nz(i) > params%vismax) Nz(i) = params%vismax
            if (Kz(i) > params%vismax) Kz(i) = params%vismax
        end do
    ...
    subroutine tke_calc(N, dt, h, Nz, P, B, tke, eps, u_taus, u_taub, z0s, tri)
      ...
      do i=1,N-1
        ...
        tri%bu(i) =  1.0_rk - tri%au(i) - tri%cu(i) + pminus(i)*dt/max(tke(i),1.0e-30_rk)
        tri%du(i) = (1.0_rk + pplus(i)*dt/max(tke(i),1.0e-30_rk)) * tke(i)
      end do
    ...
    subroutine dissipation(N, dt, h, Nz, tke, tkeold, NN, P, B, z0b, z0s, u_taus, u_taub, eps, cmue1, Lscale, tri)
      ...
      do i=1,N-1
        ...
        prod   = ce1*eps(i)/max(tkeold(i),1.0e-30_rk) * P(i)
        buoyan = cee3*eps(i)/max(tkeold(i),1.0e-30_rk) * B(i)
        diss   = ce2*eps(i)*eps(i)/max(tkeold(i),1.0e-30_rk)
        if (prod+buoyan > 0.0_rk) then
          pplus(i)  = prod + buoyan
          pminus(i) = diss
        else
      ...
      do i=1,N-1
         tri%au(i) = -2.0_rk*dt*avh(i)  /(h(i)+h(i+1))/h(i)
         tri%cu(i) = -2.0_rk*dt*avh(i+1)/(h(i)+h(i+1))/h(i+1)
         tri%bu(i) =  1.0_rk - tri%au(i) - tri%cu(i) + pminus(i)*dt/max(eps(i),1.0e-30_rk)
         tri%du(i) = (1.0_rk + pplus(i)*dt/max(eps(i),1.0e-30_rk))*eps(i) + &
