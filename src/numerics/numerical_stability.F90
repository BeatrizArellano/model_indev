module numerical_stability
  ...
  real(rk), parameter :: SAFETY      = 0.99_rk      ! Safety limit for momentum diffusion explicit and Nz fixed over the subcycle
...
pure subroutine compute_phys_subcycles(h, vismax, dt_main, n_sub, dt_sub, ierr, errmsg)
...
!
    ...
    dzmin = minval(h)
    dt_safe = (dzmin*dzmin) / (2.0_rk * vismax)

    dt_safe = SAFETY * dt_safe
